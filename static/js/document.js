...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    ...
    if (uiModule) uiModule.showToast(__('document.email_receive_only', 'Selected email account is receive-only; using your SMTP account.'));
    return null;
  }
  ...
  async function _downloadFilledPdf() {
    ...
    try {
      ...
      if (uiModule) uiModule.showError(__('document.export_failed', 'Export failed') + ': ' + e.message);
    ...
    try {
      ...
      downloadBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          overlay.querySelector('#pdf-export-status').textContent = __('common.error', 'Error') + ': ' + e.message;
          downloadBtn.disabled = false;
        }
      });
    } catch (e) {
